...
public class DoubleFactor {
	...
	Map<Assignment, DoubleDouble> matrix;
	 ...
	 */
	public DoubleFactor() {
		matrix = new HashMap<Assignment,DoubleDouble>();
	}
	...
	public DoubleFactor(DoubleFactor existingFactor) {
		matrix = new HashMap<Assignment,DoubleDouble>(existingFactor.getMatrix());
	}
	...
	public void addProbEntry (Assignment a, double value) {
		if (matrix.containsKey(a)) {
			matrix.put(a, new DoubleDouble(value, matrix.get(a).getUtility()));
		}
		else {
			matrix.put(a, new DoubleDouble(value, 0.0f));
		}
	}
	...
	public void addUtilityEntry (Assignment a, double value) {
		if (matrix.containsKey(a)) {
			matrix.put(a, new DoubleDouble(matrix.get(a).getProb(), value));
		}
		else {
			matrix.put(a, new DoubleDouble(1.0f, value));
		}
	}

	public void addEntry(Assignment a, double probValue, double utilityValue) {
		matrix.put(a, new DoubleDouble(probValue, utilityValue));
	}
	...
	public void incrementEntry(Assignment a, double probIncr,
			double utilIncr) {
		if (matrix.containsKey(a)) {
			matrix.put(a, new DoubleDouble(getProbEntry(a) + probIncr, getUtilityEntry(a) + utilIncr));
		}
		else {
			matrix.put(a, new DoubleDouble(probIncr, utilIncr));
		}
	}
	...
	public void normalise () {
		...
		matrix = new HashMap<Assignment,DoubleDouble>(probMatrix.size());
		...
		for (Assignment a : probMatrix.keySet()) {
			matrix.put(a, new DoubleDouble(probMatrix.get(a), utilityMatrix.get(a)));
		}
	}
	...
	public double getUtilityEntry(Assignment a) {
		...
		return matrix.get(a).getUtility();
	}
	 ...
	 * @return the matrix
	 */
	public Map<Assignment, DoubleDouble> getMatrix() {
		return matrix;
	}
	...
	public String toString() {
		String str = "";
		for (Assignment a : matrix.keySet()) {
			str += "P(" + a + ")=" + matrix.get(a).getProb();
			if (matrix.get(a).getUtility()!=0) {
				str += " and Q(" + a + ")=" + matrix.get(a).getUtility();				
			}
			str +="\n";
		}
		return str;
	 ...
	 *
	 */
	final class DoubleDouble {

		double probDouble;
		double utilityDouble;

		public DoubleDouble (double probDouble, double utilityDouble) {
			this.probDouble = probDouble;
			this.utilityDouble = utilityDouble;
		}
