...
The second approach developed for this experiment consist of a traditional statistical model whose parameters are estimated from the Wizard-of-Oz data set collected for the experiment.  

Figure \ref{fig:unstructuredmodel-exp3} shows the complete factored model employed to structure the dialogue domain of the experiment.  As evidenced in the figure, the model is factored in four distinct distributions: 
\begin{itemize}
\item The task completion model $P(\mathit{completed\mbox{-}task}\, | \, i_u, a_m)$ describes the probability that the current intention $i_u$ is fulfilled by the last system action $a_m$. 
\item The user goal model \\ $P(i_u' \, | \, i_u, \mathit{completed\mbox{-}task}, \mathit{perceived'}, \mathit{carried'})$ encodes the likelihood of a new user intention $i_u'$ given the current user intention $i_u$, task completion status $\mathit{completed\mbox{-}task}$, as well as the two contextual variables $\mathit{perceived}$ and $\mathit{carried}$. 
...
The four factored models remain however too large to be directly estimated from the limited amounts of data made available from the Wizard-of-Oz interactions.  We therefore introduced additional simplifying assumptions to further reduce the number of parameters associated with the model.

The first assumption serves to reduce the task completion model $P(\mathit{completed\mbox{-}task}\, | \, i_u, a_m)$ to a deterministic distribution that marks the task as completed when the executed action $a_m$ fulfils the (hidden) user intention $i_u$ and marks it as not completed  in other cases. The task completion model can therefore be defined without any parameters.
...
The second assumption pertains to the user goal model $P(i_u' \, | \, i_u, \mathit{completed\mbox{-}task}, \mathit{perceived}, \mathit{carried})$, and rests on the idea that the user intention only changes when the current intention has been completed.  The values of the two contextual variables $\mathit{perceived}$ and $\mathit{carried}$ variables are furthermore grouped into two partitions: empty set or non-empty set. These two simplifications reduce the number of parameters required for the model to four Dirichlet distributions, each with 18 dimensions (which is the number of possible user intentions). 
...
The factorisation of the utility model in Equation \eqref{eq:utilunstruct-exp3} results in a total of 296 parameters. 

\subsubsection*{Parameter estimation}

The parameter estimation follows the Bayesian learning procedure presented in Chapter \ref{chap:wozlearning}. 


\note{numbers of params}
