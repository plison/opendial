...
In order to assess the relative performance of the three dialogue management approaches presented in the previous section, we conducted a total of $3 \times 36$ user trials.

\subsection{Experimental setup}

\subsubsection*{Participants}

\note{age, gender, provenance}
\note{reward for participation}

\subsubsection*{Procedure}

\note{shuffling the order of the approaches}
\note{questionnaire right after the completion of each approach}

\note{system responsible could intervene after systematic system failures}

\subsubsection*{User expectations}

\note{show question and answers}
...
\note{no tasks completion since the notion of failure is not well defined}

\subsection{Results}

\note{to say: user turns based on ASR, duration cuts the system crashes, no repetitions requests in structured model}

\note{users feedback after experiment corroborate our claims}

\note{describe the limitations of each system (e.g. no accumulation of evidence or context-based reasoning in FSA, no good policy for states that have not been observed in the Woz interactions for unstructured model}

\note{ANOVA, since we would have two baselines and our approach? (see e.g. Passonneau's article)}

\note{how ANOVA is calculated, and post-hoc tests}
...
\end{center} 
\caption{Survey responses obtained from the participants in the user trial. }
...
\begin{sidewaystable}
\begin{tabular}{|cp{110mm}c|>{\centering}p{13mm}<{\centering}:p{13mm}:>{\centering}p{13mm}<{\centering}|p{16mm}|p{16mm}|} \hline
...
&&& \begin{footnotesize}Finite-state automata\end{footnotesize} & \begin{footnotesize}Factored statistical model\end{footnotesize} & \begin{footnotesize}Rule-structured model\end{footnotesize} & & \\ \hline \hline
...
7.97 & \textbf{2.78} & $3.8\!\times\!10^{-9}$ & $0.18$ \\ 
4.& Average number of ``$\mathrm{Disconfirm}$'' acts per dialogue &&\textbf{2.16} & 2.59 & 2.59 & \num{0.65} & $0.33$ \\ 
5.& Average number of physical movements per dialogue & & \textbf{26.68}
 & 29.89 & 27.08 & \num{0.13} &  0.80 \\ 
6.& Average number of (user and system) turns between movements &&3.63 & 3.1 & \textbf{2.54}$^{\mathbf{*}}$ & $4.\!\times\!10^{-4}$ & $2.2\!\times\!10^{-4}$ \\ 
7.& Average number of user turns per dialogue && 78.95 & 77.3 & \textbf{69.14} & \num{0.17} & 0.11 \\ 
...
11.& \ \ \textit{.. the robot reacted appropriately to your instructions?''}   && 3.70 & 3.32 & \textbf{3.86} & $7.6\!\times\!10^{-3}$ & 0.23 \\
12.&\ \ \textit{... the robot asked you to repeat or confirm your instructions ...''}   & & 2.16 & 2.19 & \textbf{3.3}$^{\mathbf{*}}$ & $1.7\!\times\!10^{-9}$ & $4.7\!\times\!10^{-7}$ \\
13.&\ \ \textit{... the robot sometimes ignored when you were speaking?''}  & & 3.24 & 2.76 & \textbf{3.43} & $6.7\!\times\!10^{-3}$ & 0.21 \\
...
\note{also evaluate the difference between the first approach tested and the other ones?}
