...
\item Verbal responses to describe the robot's current perception or tell the user that it cannot perform a requested movement due to some impeding factor (e.g. not being able to pick up an object whose location is currently not known). 
...
In order to assess the relative performance of the three dialogue management approaches presented in the previous section, we conducted a total of $3 \times 37$ user trials.
...
Before the start of the experiment, the participants were asked to fill a short survey about their age, gender, and their expectations regarding the robot verbal behaviour. The user expectations were measured with a set of six multiple-answer questions as shown in Figure \ref{fig:expectations}. The questions cover multiple aspects that affect the interaction quality, such as the (perceived) comprehension abilities of the robot (Q1), its ability to find select appropriate reactions (Q2), its use of clarification requests (Q3), its ability to distinguish between spurious recognition outputs and actual utterances (Q4 and Q5), and finally the naturalness of the interaction (Q6). One should note that the participants were told during the briefing phase to be aware of the technical limitations of the speech recogniser. The user responses must therefore be read in the light of this cautionary statement. 

The first and most evident purpose of this preliminary survey is to allow us to compare the prior expectations of the users with their actual experiences.  As shown by e.g. \cite{JokinenH06}, such comparison can yield interesting insights on the factors that empirically influence user satisfaction and their variability amongst different groups of users.  In addition, the survey also served as an indirect mean to direct the user attention towards the aspects of the robot behaviour that are of particular relevant for the experiment (namely, the robot's choice of dialogue moves). 

\subsubsection*{User trials}

After filling the survey, each participant was requested to carry out three distinct dialogues (one for each dialogue management approach).  The order in which the approaches were tested was shuffled before each trial to mitigate the effects of the ordering sequence on the results.\footnote{The interactions become indeed increasingly efficient as the user gets more accustomed to the robot and the task to perform.  The average duration of the first dialogue amounts to 6'31, while the average duration of the second dialogue is reduced to 5'08, and the duration of the last dialogue goes down even further to 4'40.}  The participants did not have a time limit to complete the task.  In some cases, the interactions had to be temporarily interrupted due to hardware failures (the downtime was in such case subtracted from the dialogue duration). Immediately after each dialogue, the participants were asked to fill a small survey to determine how they subjectively perceived the interaction.  The survey was once more divided into six multiple-answer questions (cf. below).  

All interactions were captured on video, totalling about 12 hours of footage. In addition to the videos, the dialogue system also logged all user and system turns occurring in the interaction along with their associated dialogue state (similarly to the Wizard-of-Oz data described in Section \ref{sec:wozcollection-exp3}). 
...
In line with previous work on the evaluation of spoken dialogue systems \citep[see e.g.][for an overview]{2009Jokinen}, we defined a set of 9 metrics aimed at measuring the quality and efficiency of the recorded dialogues:
\begin{enumerate}
\item Average number of repetition requests per dialogue
\item Average number of confirmation requests per dialogue
\item Average number of repeated instructions per dialogue
\item Average number of ``$\mathrm{Disconfirm}$'' user dialogue acts per dialogue
\item Average number of user turns per dialogue
\item Average number of system turns per dialogue
\item Average number of physical movements per dialogue
\item Average number of (user and system) turns between movements
\item Average duration of each dialogue (in minutes)
\end{enumerate}

Although many evaluation schemes also include measures of task completion in their metrics, we found such measure to be difficult to incorporate in our experimental setup, as the participants did not really have the possibility to end a task before its conclusion. Although some dialogues had to be terminated before the full completion of the task, these cases were due to hardware failures (e.g.\ system crash or physical collapse) and were not caused by the dialogue manager.
 
The metrics were calculated on the basis of the interaction logs produced by the dialogue system during the course of the dialogue. As the interaction logs are generated automatically, it should be noted that the user turns correspond to the user inputs produced by the speech recogniser and do not necessarily correspond to the actual user turns in the dialogue (due to spurious recognition results caused by ambient noise).

The empirical results obtained for these metrics are shown in Figure \ref{fig:objective}. One can observe that the rule-structured model produces on average shorter and more efficient dialogue than the two baselines.  In particular, on the basis on its optimisation on the Wizard-of-Oz training set, the rule-structured model learned to prefer confirmation to repetition requests, which are less annoying to the user and provide better feedback. The number of system turns is also notably shorter than for the finite-state and factored statistical model, reflecting the fact that the rule-structured model is better able to filter out spurious recognition results. 
...
The subjective metrics reflect the answers to the six multiple-answer questions specified in the user survey. Each question allows for five alternative answers scale ranked from worst to best.  The six questions were:
\begin{enumerate}
...
Figure \ref{fig:subjective} details the user responses to these six questions as a function of the dialogue management approach.  As one can observe, the responses to the survey do seem to indicate that the rule-structured model was perceived to produce more effective and natural conversational behaviours. 
...
The empirical results derived from the user trials demonstrate the benefits of a hybrid modelling framework based on probabilistic rules compared to traditional approaches. Through a follow-up error analysis, we found this difference to be imputable to several factors:
\begin{itemize}
\item Although the finite-state automaton functioned well in most trials, it often required multiple repetitions in order to reach sufficient confidence regarding the instruction to perform.  In comparison, the rule-structured model was able to accumulate evidence from several information sources (including the prior probabilities of the various instructions given the context) thanks to an explicit, probabilistic representation of the user intention $i_u$.  This possibility to accumulate evidence and exploit contextual knowledge enables the rule-structured model to zero in on the right instruction faster and more efficiently. One can observe this difference in the metrics 1 (number of repetition requests), 6 (number of turns per instructions) and 12 (frequency of clarification requests) of the user trial.  

\item The factored statistical model was able to operate well in the most common conversational situations, but was frequently led astray when encountering events that had not been observed in the Wizard-of-Oz training set. In particular, although the user action model $P(a_u'\, | \, i_u', a_m)$ offered reasonable estimates for the most prevalent user intentions and dialogue acts, this was not the case for less common instructions which had few or no occurrences in the training set.  This disparity in the estimated distributions often resulted in the system getting stuck in unseen dialogue states and selecting suboptimal actions.  Although it was trained on the exact same data as the factored statistical model, the rule-structured approach did not suffer from this data sparsity in the same extent due to its more powerful generalisation abilities. This difference can be clearly noticed in the metrics 3 (number of repeated instructions), 9 (average duration), 10 (quality of the robot's understanding) and 11 (appropriateness of the robot's conversational behaviour). 

\item The finite-state automaton and the factored statistical model also encountered difficulties in filtering out spurious speech recognition hypotheses, especially the ones produced by the robot's own noise when executing physical movements.  This often led the robot to request the user to repeat an instruction when no instruction was actually uttered.  The rule-structured model proved to be better at filtering out these unwanted results.\footnote{The finite-state automaton employs a slightly higher threshold for moving from state $s_1$ to $s_3$ whenever the variable $\mathit{motion}$ was set to true. Although this mechanism did help in reducing the number of spurious results, it remained insufficient to fully distinguish between actual user utterances and erroneous recognitions.}  The metrics 8 (number of system turns) and 14 (ability of the robot to distinguish noise from speech) illustrate this discrepancy. 


\item Finally, it worth noting that the rule-structured model systematically preferred to employ confirmation requests (\utt{should I do X?}) rather than repetition requests (\utt{sorry, could you repeat?}) when faced with unclear user instructions, as evidenced by the metric 1. While this  discard of repetition requests in the learned model came to us as a surprise (since the wizard did use some repetition requests in the Wizard-of-Oz interactions, although admittedly less than confirmation requests), it proved to be an excellent conversational strategy for the dialogue domain, as participants perceived confirmation requests to be much more natural and informative that repetition requests. By contrast, the two baseline approaches relied much more heavily on repetition requests to elicit the user instructions.
\end{itemize} 

\subsubsection*{Interpretation of the results}

The empirical results obtained from the user trials show that the hybrid modelling framework presented in this thesis is a viable and competitive alternative to classical dialogue management approaches.  The results are however subject to two caveats that should be mentioned.  Most importantly, we want to stress that the results obtained for the two baselines are contingent on the particular design choices described in Section \ref{sec:approach1} and \ref{sec:approach2}.  Although these design choices are in our view relatively uncontroversial and do follow standard practices in the field of dialogue management,\footnote{ Factored statistical models are indeed employed in most MDP and POMDP-based approaches to dialogue management, as we saw in Chapter \ref{chap:probmodelling}.} there is no doubt that these approaches could be further improved through more sophisticated optimisation methods and parameter tuning. The intended purpose of the user evaluation was however \textit{not} to compare probabilistic rules with the ``best possible'' handcrafted and statistical techniques, as such characterisation would make little sense and remain impossible to prove in practice. Instead, the empirical results presented in the previous pages are best interpreted as a comparison between approaches featuring (approximately) the same level of sophistication in their representation of the dialogue domain, but differing with one another along two distinct dimensions:
\begin{itemize}
\item The finite-state automaton and the rule-structured model share the same domain knowledge and mappings between user instructions and corresponding system actions.  They however differ in their account of uncertainty: while the finite-state relies on confidence thresholds, the rule-structured model uses probabilistic reasoning to infer the most likely instruction given the observed inputs, prior likelihoods and external context.
...
Furthermore, we also want to highlight the fact that the results were obtained in a particular dialogue domain that exhibits (1) high levels of noise and uncertainty, (2) a rich relational structure and (3) limited amounts of available training data.  Although these three characteristics are common to many  dialogue domains, one should be wary of extrapolating these results to domains that do not share these properties.  For instance, dialogue domains for which most state variables are fully observable might be better addressed by purely symbolic techniques instead of relying on a hybrid approach. Similarly, dialogue domains that are more weakly structured and/or can draw on large amounts of in-domain training data might not have a direct need for the expert domain knowledge incorporated in probabilistic rules.   


\subsubsection*{Comparison with initial user expectations}

To conclude the analysis of the empirical results obtained from the user trial, we found it useful to contrast the results obtained from the survey with the initial user expectations, as shown in Table \ref{table:compexpectation-exp3}.  We can observe that for all six questions, the actual system behaviour exceeded the initial expectations of the user. It should however be again mentioned that the participants were made aware of the technical limitations of the robot's speech recogniser during the presentation of the task, and that this fact may have had an influence on their expectations regarding the robot's conversational abilities. 

\begin{table}
\begin{tabular}{|cp{60mm}c|r:r|p{18mm}|p{0mm}} \cline{1-6}
&\centering \vspace{-2mm}Metrics & & \multicolumn{1}{p{20mm}:}{\centering \vspace{-2mm}Initial expectations} & \multicolumn{1}{p{20mm}|}{\centering Rule-structured model} &\centering \vspace{-2mm}$p$-values ($t$-test) & \\ \cline{1-6}
& \vspace{-2mm} \textit{``Did you feel that ...}  &&  & & & \\ 
1.&\ \ \textit{... the robot correctly understood what you said?''}  &&  3.22 &  \textbf{3.68}$^{\mathbf{*}}$ &   0.025  & \\
2.& \ \ \textit{.. the robot reacted appropriately to your instructions?''}   && 3.35 & \textbf{3.86}$^{\mathbf{*}}$ & 0.004 & \\
3.&\ \ \textit{... the robot asked you to repeat or confirm your instructions ...''}   & & 2.68 & \textbf{3.30}$^{\mathbf{*}}$ & $4.6 \times 10^{-4}$ &  \\
4.&\ \ \textit{... the robot sometimes ignored when you were speaking?''}  &  & 2.89 & \textbf{3.43}$^{\mathbf{*}}$ & 0.015 & \\
5.&\ \ \textit{... the robot sometimes thought you were talking when you were not?''}  & & 3.81 & \textbf{4.41}$^{\mathbf{*}}$ &  0.001 & \\
6.&\ \ \textit{... the interaction flowed in a pleasant and natural manner?''} \vspace{3mm}  & & 2.92 &  \textbf{3.32}$^{\mathbf{*}}$ & 0.041 &  \\ \cline{1-6}
\end{tabular} \vspace{3mm}
\label{table:compexpectation-exp3}
\caption{Comparison between the initial user expectations and the actual performance of the rule-structured model in the trials. The $\mathbf{*}$ symbol indicates better-than-expected behaviour with a significance level $\alpha = 0.05$. }
\end{table}

\section{Conclusion}

This chapter presented an extensive user evaluation of the modelling framework developed in this thesis.  The experiment was designed to compare a dialogue manager relying on probabilistic rules against two baselines that respectively correspond to a purely handcrafted dialogue manager (expressed as a finite-state automaton) and a purely statistical dialogue manager (based on factored models). 
