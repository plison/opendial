...
We conducted several experiments to assess the validity of our approach in three distinct learning scenarios: \begin{enumerate}
...
We carried out all the experiments described in this thesis in a \textit{human--robot interaction} \index{human--robot interaction} (HRI) domain.  The selection of this application domain as a test bed for our framework was motivated by two factors.  First, HRI domains often include a rich situated environment and a complex collection of tasks to complete by the agent. Neither the environment nor the tasks are static entities -- physical objects can for instance be moved from one placed to another, and the list of tasks is regularly altered as a result of the coordinated actions of the robot and human user. Moreover, HRI domains are frequently associated with significant levels of uncertainty due to imperfect sensors, unreliable motors, and failure-prone speech recognition.\footnote{For practical reasons, the microphones are often placed on the robot itself, at a significant distance from the speaker.  This distance between source and receiver is a major degradation factor in speech recognition \citep{wolfel2009distant}.  Moreover, the microphones are also adjacent to a number of mechanical motors which may disturb the sound signal and lead to spurious detections.} Human--robot interactions therefore embody both a rich conversational context and high levels of uncertainty, which make them an ideal test bed to evaluate the performance our modelling approach in real settings.
