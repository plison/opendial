\chapter{Probabilistic models of dialogue management}
\label{chap:probmodelling}

The previous chapter surveyed the most important approaches to dialogue management, and outlined in particular the benefits of statistical techniques to account for the uncertainty and unpredictability inherent to dialogue. The present chapter exposes the theoretical and methodological foundations of these statistical approaches and their application to dialogue management. 
...
The \opendial toolkit we have developed for this thesis includes two inference algorithms: generalised variable elimination \citep[][p. 1103]{Koller+Friedman:09} and a specific type of importance sampling algorithm called likelihood weighting, which we describe below. These algorithms are used in the processing workflow of the dialogue manager to (1) update the dialogue state upon the reception of new observations and (2) select system actions on the basis of this updated dialogue state (the details of this workflow is provided in Section \ref{sec:processing-workflow}).  
...
A large number of samples can be collected in such manner. The resulting posterior distribution for a set of query variables $\mathbf{Q}$ can be derived from the samples by normalising the weighted counts associated with each value of the query variables, as shown in Algorithm \ref{algo:lw}.  The notation $\mathbf{x}(\mathbf{Q})$ is used to refer to the values specified for the variables $\mathbf{Q}$ in the assignment $\mathbf{x}$. Algorithm \ref{algo:lwu} extends the procedure to the calculation of utility distributions. In this case, the utility values are not normalised but correspond to a weighted average of the sampled utilities. 
...
\end{algorithmic}
\caption{\textsc{Likelihood-Weighting-Util} $(\mathcal{B},\mathbf{Q}, \mathbf{E}=\mathbf{e}, N)$}
\label{algo:lwu}
\end{algorithm}
