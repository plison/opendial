...
A large number of samples can be collected in such manner. The resulting posterior distribution for a set of query variables $\mathbf{Q}$ can be derived from the samples by normalising the weighted counts associated with each value of the query variables, as shown in Algorithm \ref{algo:lw}. Algorithm \ref{algo:lwu} extends the procedure to the calculation of utility distributions. In this case, the utility values are not normalised but correspond to a weighted average of the sampled utilities. 
