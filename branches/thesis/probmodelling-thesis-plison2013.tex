...
The most well-known model-based learning algorithm is \textit{value iteration}\index{value iteration}, which operates by estimating the value function via a sequence of updates based on Bellman's equation. Given a value function estimate $V_k$ available at step $k$, the estimate at step $k+1$ is calculated as: 
\begin{equation}
...
\textit{Temporal-difference} methods such as Q-learning\index{Q-learning} \citep{watkins92} and SARSA \citep{rummery:phd95} are amongst the most popular model-free techniques.\index{temporal-difference} These algorithms rely on Bellman's equation to incrementally improve the action--value estimates on the basis of the rewards resulting from the agent actions. Temporal-difference methods\index{temporal-difference methods} are also called ``boostrapping'' methods as they approximate new $Q$-value estimates based on previous estimates. 
