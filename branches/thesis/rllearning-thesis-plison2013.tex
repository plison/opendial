...
We now assume the effect distribution associated with the predictive rule $r_{11}$ to be initially unknown and replaced by parameters, leading to the following rule: \vspace{-2mm}
\begin{align*}
r_{11}: \ \ & \forall y, \\ 
...
The model-free approach relied on a utility model structured with 11 probabilistic rules associated with 26 Gaussian parameters. As the model-free approach does not estimate an explicit transition model, the user intention is derived in a deterministic manner on the basis of the recent history of dialogue acts.  The utility rules define the action utilities based on this user intention and on the general dialogue context, as listed in Appendix \ref{chap:domainspecs} (Section \ref{sec:domainspecs-rrlearning}). 

The resulting rule parameters are optimised using the SARSA-based learning method outlined in Section \ref{sec:modelfree}. 
