...
<Condition> ::= <AtomicCondition> | <ComplexCondition> | $\emptyset$
...
The formalisation allows specific elements $y_1, \dots, y_p$ inside the conditions and effects of a rule to be \textit{underspecified}.\index{probabilistic rule!quantified variables in}  The mapping between conditions and effects specified by the rule is therefore duplicated for every possible assignment of the underspecified variables.  Section \ref{sec:applicationquantif} explains how this finite set of possible assignments (also called \textit{groundings}) is practically determined on the basis of the current state. Based on this quantification mechanism, probabilistic rules can cover large portions of the state space in a highly compact manner, based on a reduced number of parameters. One of the main advantages of this representation is that it allows for powerful forms of \textit{parameter sharing}\index{parameter sharing}, as the effect probabilities $p_{(i,j)}$ in the above rule are made independent of the various instantiations of the variables $y_1, \dots, y_p$. 

It should be stressed that, although the quantification mechanism employed in the presented framework makes use of conceptual tools borrowed from first-order logic, the quantification mechanism presented here essentially serves as ``syntactic sugar'' on top of a classical probabilistic model and does not claim to equate to the expressive power (and theoretical characterisation) of quantifiers in first-order logic\index{first-order logic}.  The quantification mechanism adopted in this thesis is similar to the one used in Markov Logic Networks \citep{Richardson:2006} and other frameworks within statistical relational learning \citep{getoor:srlbook07}, with the constraint that universal quantifiers are in this framework only allowed at the top of the rule.
...
\FORALL {variable $A \in A_1, \dots, A_l$}
...
For a utility rule $r$ with input variables $I_1, \dots, I_k$ and decision variables $A_1, \dots, A_l$, the rule utility for the grounding $\mathbf{g}_j$ is defined as:
 \begin{align}
& U_{\mathbf{g}_j}(I_1\!=\!i_1, \dots, I_k\!=\!i_k, A_1\!=\!a_1[\mathbf{y} / \mathbf{g}_j], \dots A_l\!=\!a_l[\mathbf{y} / \mathbf{g}_j] ) \label{eq:quantifuruledistrib} \\ 
...
\begin{align}
& U_{r}(I_1, \dots, I_k, A_1\, \dots, A_l) = \sum_{j=1}^{|G|} U_{\mathbf{g}_j}(I_1, \dots, I_k, A_1, \dots, A_l) \label{eq:quantifuruledistrib2}
\end{align}


The instantiation of rule $r_4$ given the input variables $\mathit{task}$, $\mathit{graspable}(o_1)$ and $\mathit{graspable}(o_2)$ is shown in Figure  \ref{fig:quantinstantitionutil}.  The utility distribution $U_{r_4}(\mathit{task}\!=\!\mathit{grasp}(o_1), \mathit{graspable}(o_1) \!=\!\mathit{true},$ $\mathit{graspable}(o_2)\!=\!\mathit{false}, a_m)$ assigns for instance the action $a_m\!=\!\mathit{grasp}(o_1)$ to a utility of 2, while $a_m\!=\!\mathit{grasp(o_2)}$ is assigned to a utility of -2 in this situation.
...
\vspace{-2mm}
\caption{Equivalence node $\mathit{eq}_{X}$ with parents $X$ and $X_p$.}
\label{fig:prediction}
\end{wrapfigure}

Several techniques are available to practically encode such kind of evidence \citep{Pan06beliefupdate}. The method adopted in this thesis is to add a new boolean-valued chance node, subsequently called the \textit{equivalence node}\index{equivalence node} $\mathit{eq}_{X}$, that is conditionally dependent on both $X$ and $X_p$, as shown in Figure \ref{fig:prediction} (the double circle denotes a deterministic distribution). The use of a distinct node to express the evidence is motivated by the fact that $X$ and $X_p$ may have arbitrary incoming and outgoing edges with other variables. 
...
As in most frameworks within statistical relational learning \citep{getoor:srlbook07} and probabilistic inductive logic programming \citep[see e.g.][]{Poole:2008}, probabilistic rules follow the standard assumptions of database semantics\index{database semantics}, namely the unique-names assumption (every term employed in the dialogue state refers to a distinct entity), the closed-world assumption (what is not explicitly specified in the state is assumed to be false), and the domain closure (the state does not contain more elements than the one explicitly specified). The work of \cite{MilchEtAl_ijcai05} on open-universe probability models\index{open-universe probability models} constitutes an interesting exception to this rule but remains at an early state of development. Extending the modelling framework presented in this thesis to allow for open-universe models would certainly be an interesting area for future work. 
