...
Each \textit{if ... then} branch in the rule specifies both a condition on particular state variables, and an associated distribution over possible effects.   The \textit{if ... then ... else} structure is read in sequential order, as in programming languages, until a satisfied condition is found, which causes the activation of the corresponding probabilistic effects. 
...
A terminological note is here in order: we shall use the term \textit{probabilistic rules} as an umbrella term that covers all types of rules defined in this thesis, while \textit{probability rules} will only refer to rules defining probability distributions over effects, and \textit{utility rules} will refer to rules defining utility distributions.
...
%It often happens that the distribution $P(E_i)$ only includes one single effect with probability 1.  In such case, the rule corresponds to a deterministic distribution. 
%In the latter case, we adopt a Bayesian learning approach (cf. Section \ref{sec:learning}) and assume the probabilities $e_i^{1...m_i}$ to be drawn from the conjugate prior of categorical distributions, namely Dirichlet distributions. 
...
The rule distributions directly reflect the rule semantics.  Formally, the conditional probability distribution of a rule $r$ given its input variables $I_1,...I_k$ is defined as: 
\begin{align}
...
\begin{algorithm}[h!]
\caption{: \textsc{InstantiateProbRule} ($\mathcal{B}, \mathit{rule}$)}
\begin{algorithmic}[1] \vspace{1mm}
\REQUIRE $\mathcal{B}$: Bayesian network for the current state
\REQUIRE $\mathit{rule}$: Probability rule to instantiate in network  \vspace{1mm}
\STATE $I_1,...I_k \leftarrow$ input variables for $\mathit{rule}$
...
\begin{algorithm}[h!]
\caption{: \textsc{InstantiateUtilRule} ($\mathcal{B}, \mathit{rule}$)}
\begin{algorithmic}[1] \vspace{1mm}
\REQUIRE $\mathcal{B}$: Bayesian network for the current state
\REQUIRE $\mathit{rule}$: Utility rule to instantiate in network  \vspace{1mm}
...
\ENDIF
\STATE Add in $Val(A')$ the action values specified for it in the effects of $r$
...
\begin{algorithm}[h!]
\caption{: \textsc{GetInputVariables} ($\mathcal{B}, \mathit{rule}$)}
\begin{algorithmic}[1] \vspace{1mm}
\REQUIRE $\mathcal{B}$: Bayesian network for the current state
\REQUIRE $\mathit{rule}$: Probability or utility rule  \vspace{1mm}
\STATE $\mathcal{I}_{r} \leftarrow $ Initial (possibly underspecified) input variables for $\mathit{rule}$
...
\begin{algorithmic}[1] \vspace{1mm}
\STATE $I_1,...I_k \leftarrow $ \textsc{GetInputVariables} ($\mathcal{B}, \mathit{rule}$)
\end{algorithmic}
...
Given an input assignment $\{I_1\!=\!i_1 \land ... I_k\!=\!i_k\}$, the set of groundings $G$ are determined in a heuristic manner, by checking which ground terms (constants and functions of constants) from the input assignment can function as substitutions for the quantified variables.  These ground terms define the domain of discourse for the application of the universal quantifier. The collection of relevant ground terms is then combined into subsets of size $p$, where $p$ corresponds to the number of universally quantified variables $\mathbf{y} = y_1, ... y_p$. These combinations form the groundings $G$ for the input assignment. 
...
%\begin{algorithm}[h!]
%\caption{: \textsc{GetGroundings} ($\mathit{rule}, \{I_1\!=\!i_1 \land ... I_k\!=\!i_k\}$)}
%\begin{algorithmic}[1] \vspace{1mm}
%\REQUIRE $\mathit{rule}$: Probability or utility rule 
%\REQUIRE $\{I_1\!=\!i_1 \land ... I_k\!=\!i_k\}$ assignment for the parent variables of the rule  \vspace{1mm}
%\STATE $G \leftarrow \emptyset$
%\FORALL {condition $c_i$ in $\mathit{rule}$}
