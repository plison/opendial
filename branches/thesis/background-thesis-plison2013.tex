...
The control of a complete dialogue system is sometimes decomposed into multiple behavioural layers, each responsible for a specific aspects of the interaction. \cite{Allen:2000:AGD:973935.973937} makes an explicit distinction between task management (in charge of monitoring and advancing the execution of the system objectives) from dialogue management \textit{stricto sensu} (in charge of the more conversational aspects of the interaction). Similarly, the Olympus/Ravenclaw architecture described by \cite{Bohus:2009} includes both an interaction manager for the low-level control of the conversational floor and a dialogue manager for the higher-level dialogue decisions.  Establishing the exact boundary between these processes is, however, not always trivial.
