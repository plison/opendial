...
The control of a complete dialogue system is sometimes decomposed into multiple behavioural layers, each responsible for a particular aspect of the interaction. \cite{Allen:2000:AGD:973935.973937} divide for instance the task of controlling the dialogue into various agents (discourse manager, plan manager, behavioural agent, etc.), each agent being in charge of a specific type of decision. Similarly, the Olympus/Ravenclaw architecture described by \cite{Bohus:2009} includes both an interaction manager for low-level control of the conversational floor and a dialogue manager for the higher-level dialogue decisions.  Establishing the exact boundary between these modules is, however, not always trivial. This thesis adopts a broad definition of dialogue management and views the dialogue manager as the central decision-making process of the dialogue architecture. 
