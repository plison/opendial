...
The chapter is divided in two sections.  The first section presents the design of the \opendial toolkit.  The section surveys the toolkit workflow, the specification of dialogue domains in XML, and explains how the algorithms employed for approximate inference, sampling and forward planning are practically implemented. We also briefly compare the toolkit to other other available architectures and discuss its current limitations.  The second section shows how the toolkit is integrated and extended into a full-fledged dialogue system for human-robot interaction. The section describes the general system architecture (built on top of \opendial) as well as the individual components developed to control the verbal and non-verbal behaviour of the robot.  
...
\begin{itemize}
\item \textit{Categorical distributions}:  Sampling a categorical distribution is done through inverse random sampling, following the method described in \cite[][p. 489]{Koller+Friedman:09}. When constructing the distribution, the system sorts the variable values (according to e.g. lexicographic order), and calculate the cumulative density function (CDF) relative to this order.  Sample values are then extracted at runtime by (1) generating a pseudo-random float number between 0 and 1 and then (2) locating the greatest number in the CDF that is less than or equal to the number just generated.  The last operation is done through binary search. The value corresponding to this number is then selected as the sample.
...
\item \textit{Normal distributions}:  Normal distributions are sampled in \opendial via the well-known Box-Muller method \citep{rBOX58a}, which allows us to produce two sample values for the normal distribution based on two pseudo-random numbers.
...
In addition to the \opendial core components -- constituted by the dialogue state and its associated update mechanism based on probabilistic rules -- the full dialogue system used in our experiments also integrated additional modules for speech recognition and synthesis, robot perception and motor control.  The generic architecture for the system is shown in Figure \ref{fig:impl_architecture}. 
