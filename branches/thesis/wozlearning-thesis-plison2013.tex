...
One key element of the learning cycle is the definition of the probability $P_{\mathcal{B}_i}(a_i\,; \boldsymbol\theta)$, which specifies the likelihood of the wizard action $a_i$ in a dialogue state $\mathcal{B}_i$ given the parameters $\boldsymbol\theta$. The purpose of this likelihood is intuitively to favour the parameter values that provide a good fit for the wizard action choices. In practice, this is achieved by defining the likelihood of the wizard action in terms of the relative utility of this action compared to the alternative choices.  More precisely, the probability of the wizard action will depend on its position in the ranked list of possible actions (in descending order).  The action at the top of this ranked list will thus be considered as the one that is most likely to be selected by the wizard. Formally, the likelihood of action $a_i$ given the parameters $\bold\theta$ is defined as:
\begin{align}
...
\cdots
\end{cases} \nonumber \\\vspace{2mm}
& = (1-p)^x p \ \ \ \ \ \ \ \ \ \ \text{ where } x \text{ is the position of action } a_i \label{eq:likelihood1} \\ 
&  \phantom{= (1-p)^x p} \ \ \ \ \ \ \ \ \ \ \ \text{ in the ranked list of actions}  \nonumber
\end{align}

The $\alpha$ factor in Eq. \eqref{eq:likelihood1} represents as usual the normalisation factor, while the probability $p$  represents the learner confidence in the judgement of the wizard.  A value $p = 0.7$ will hence indicate that the wizard is assumed to be rational -- and thus select the highest-utility action -- with probability $0.7$. The probability $p$ indirectly defines the learning rate of the estimation process: the higher the probability, the faster the learner will converge to a policy that imitate the Wizard actions.  On the other end, a high value for $p$ makes the learner more vulnerable to the occasional errors and inconsistencies of the wizard. 

Equation \eqref{eq:likelihood1} is a particular instance of a geometric distribution.\footnote{Technically speaking, the definition in Equation \eqref{eq:likelihood1} corresponds to a truncated version of the geometric distribution, as the support of the distribution is finite in our case (as the number of possible actions is bounded). The reduction of the distribution to a finite support is achieved via the normalisation factor $\alpha$.}.  Geometric distributions represent the number of Bernoulli trials required to get one success. The use of a geometric distribution is motivated by the presupposed rationality of the wizard, as we assume that she/he will select the highest utility action with probability $p$, the second-highest action with probability $(1-p)p$, and so forth. The geometric distribution is monotonically decreasing, thus ensuring that the probability of a high-utility action will always be higher than its lower-utility alternatives.  
