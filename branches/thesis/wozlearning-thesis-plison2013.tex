...
where $ \mathrm{B}(\boldsymbol\alpha)$ serves as normalisation factor and relies on the gamma function $\Gamma$.\footnote{The gamma function is a generalisation of the factorial for real numbers, and is defined as $\Gamma(z) = \int_0^\infty  t^{z-1} e^{-t}\,{\rm d}t$.} We remind the reader that the notation $p(\theta_1,...\theta_k\,; \boldsymbol\alpha)$ refers to the density function of the $\theta_1,... \theta_k$ random variables given the specified (hyper-)parameters $\boldsymbol\alpha$. 
